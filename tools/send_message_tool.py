...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    """
    # Signal platform was removed in the programmer-focus trim. The
    # supporting gateway.platforms.signal_rate_limit module no longer
    # ships with this build, so this dispatch path returns a clear
    # error instead of importing a missing module. Restore the signal
    # adapter to re-enable sending.
    return {"error": "Signal platform is not available in this build"}
